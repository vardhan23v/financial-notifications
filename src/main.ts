...
import { recordMetric, startMetricsServer, stopMetricsServer } from "./metrics";
import { resolveChannels } from "./channels";
import { dispatch, getDLQ } from "./processor";
import { alertOnDLQ, setRemediateHook } from "./monitor";
...
async function notificationHandler(event: object): Promise<void> {
  const evt = event as DemoEvent;
  const channels = await resolveChannels(evt, userPreferences);
  ...
  log.info(
    { eventId: evt.id, type: evt.type, channels },
    "Dispatching event",
  );
  ...
  if (evt.type === "margin_call") {
    ...
    recordMetric("notifications_failed_total", 1, { channel: channels[0], type: evt.type });
    throw new Error(errMsg);
  }

  // Simulate successful delivery for other event types
  recordMetric("notifications_sent_total", 1, { channel: channels[0], type: evt.type });
...
setRemediateHook(async (event: object): Promise<boolean> => {
  const evt = event as DemoEvent;
...
async function main(): Promise<void> {
  ...
  for (const event of events) {
    ...
    await dispatch(event, notificationHandler, 3);
  }

  // --- DLQ Inspection ---
  const dlqEntries = getDLQ();
  recordMetric("dlq_size", dlqEntries.length);
  ...
  for (const entry of dlqEntries) {
    ...
    log.info(
      { eventId: (entry.event as DemoEvent).id, remediated: result },
      "Remediation result",
    );
  }

  // --- Circuit breaker & provider health (simulated) ---
  recordMetric("circuit_breaker_state", 0, { provider: "sms" });
  recordMetric("circuit_breaker_state", 0, { provider: "email" });
  recordMetric("provider_health", 1, { provider: "sms" });
  recordMetric("provider_health", 1, { provider: "email" });
  recordMetric("consumer_lag", 0, { topic: "notifications", partition: "0" });
